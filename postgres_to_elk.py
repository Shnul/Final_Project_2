...
def insert_exchange_rates(data: Box) -> None:
...
def create_consumer() -> KafkaConsumer:
    ...
    try:
        consumer = KafkaConsumer(
            'exchange_rate_topic',  # New topic for indexing data into Elasticsearch
            bootstrap_servers=['course-kafka:9092'],
            value_deserializer=lambda x: Box(json.loads(x.decode('utf-8'))),
